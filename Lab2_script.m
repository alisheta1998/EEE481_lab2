...
%SNR = Eb_No_db = 10*log(Eb/No) = 0 ;
%Eb = 1 ;
%No = Eb / e^0 ;
No = Energy_per_bit / exp(0/10);
...
x_square = GenerateSquarePulses(t_axis, T_sq, Energy_per_bit, fs, x_bits, 'unipolar'); 
y_square = AWGNChannel(x_square, No, fs);
[rec_bits, ht, z_square, D_samples] = MatchedFilter(T_sq, Energy_per_bit, fs, y_square, 'unipolar'); 

BER_uni = ComputeBER(x_bits, D_samples(1: N_bits));
%%%
...
y_square = AWGNChannel(x_square, No, fs);
[rec_bits, ht, z_square, D_samples] = MatchedFilter(T_sq, Energy_per_bit, fs, y_square, 'bipolar'); 

BER_bi = ComputeBER(x_bits, D_samples(1: N_bits));
%%%
...
for i = 1 : length(Eb_No_dB_vector)
    ...
    y_square = AWGNChannel(x_square, No, fs);
    [rec_bits, ht, z_square, D_samples] = MatchedFilter(T_sq, Energy_per_bit, fs, y_square, 'unipolar'); 
    BER_uni(i) = ComputeBER(x_bits, D_samples(1: N_bits));
    %Pi_Polar
    x_square = GenerateSquarePulses(t_axis, T_sq, Energy_per_bit, fs, x_bits, 'bipolar'); 
    y_square = AWGNChannel(x_square, No, fs);
    [rec_bits, ht, z_square, D_samples] = MatchedFilter(T_sq, Energy_per_bit, fs, y_square, 'bipolar'); 
    BER_bi(i) = ComputeBER(x_bits, D_samples(1: N_bits));
end
