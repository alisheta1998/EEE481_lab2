...
x_square=zeros(1,N); % x_square is the same length as t-axis to gurante a successful plot
switch type
    case ('bipolar')
...
%             if x_bits(i) == 0
%                 uni_one_square = horzcat(uni_one_square,zeros(1,N_sq));
%             else
%                 uni_one_square = horzcat(uni_one_square,one_square);
%             end
%         end
%         x_square(1:length(uni_one_square))= (uni_one_square.*2) -1 ;
        
        %Second Implementation : 
        L_x_bits = length(x_bits);
        v1 = repmat(one_square, 1, L_x_bits);
        Seq_L = length(v1);
        v2 = (x_bits .* 2) - 1;
        v2 = repelem(v2, 1, round(Seq_L / L_x_bits));
        x_square(1: Seq_L) = v1 .* v2;
    ...
    case ('unipolar')
        %%% This case is for RZ
        %%% WRITE YOUR CODE HERE
        % First implementation (doesn't work with more than 1 x_bit)
        %uni_one_square = repmat(one_square,1,x_bits);
        % x_square(1:length(uni_one_square))= uni_one_square ;
        
        % Second implementation (Workes with uni but not with bi :( )
        A = sqrt( ( 2 * E_bit ) / N_sq  );
        one_square = A * one_square ;
        uni_one_square = [];
        for i=1:1:length(x_bits)
